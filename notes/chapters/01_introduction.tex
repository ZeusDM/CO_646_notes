...
A matrod of the form \(M(A)\) for some \(A \in \mathbf{F}^{R \times E}\) is called \vocab{\(\mathbf{F}\)-representable} (a.k.a. linear over \(\mathbf{F}\), for the algebra people).

A \vocab{matrix} \(A\) in \(\mathbf{F}^{R \times E}\) is a function \(A \colon R \times E \to \mathbf{F}\).
